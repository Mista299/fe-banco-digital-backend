#!/bin/bash
# Variables y helpers compartidos — HU-09 Depósito autenticado

source "$(dirname "${BASH_SOURCE[0]}")/../config.sh"
URL_BASE="${API_BASE}/api/v1"
COOKIES="$(dirname "${BASH_SOURCE[0]}")/cookies_hu09.txt"
...
registrar_usuario() {
  ...
  local doc="8${ts: -8}" email="hu09${ts}${suf}@nexus.co" user="hu09${ts}${suf}"
  local resp
...
resumen() {
  echo ""
  echo "========================================================"
  printf "  Resultado HU-09: %d pasaron · %d fallaron\n" "$PASS" "$FAIL"
  echo "========================================================"
